...
const Portfolio = () => {
  ...
  const totalHoldingsValue = holdings?.reduce((acc, h) => acc + (h.value || 0), 0) || 0;
  const totalValue = (virtualBalance || 0) + totalHoldingsValue;
  const totalPnl = holdings?.reduce((acc, h) => acc + (h.pnl || 0), 0) || 0;
  const totalSharesPurchased = holdings?.reduce((acc, h) => acc + (h.shares || 0), 0) || 0;
  ...
  return (
    <div className="min-h-screen bg-background pt-24 pb-12">
      ...
      <div className="max-w-6xl mx-auto px-4">
        {error && (
          <div className="mb-8 p-4 bg-red-50 border border-red-200 text-red-600 rounded-2xl flex items-center space-x-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
          <div className="glass-card rounded-3xl p-8 bg-blue-600 border-none shadow-xl shadow-blue-500/20">
            ...
            {loading ? <Skeleton className="h-8 w-32 bg-blue-400" /> : <h2 className="text-3xl font-black text-white">₹{totalValue.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</h2>}
          </div>
          ...
          <div className="glass-card rounded-3xl p-8 bg-white border-slate-100 shadow-sm">
            ...
            {loading ? <Skeleton className="h-8 w-32" /> : <h2 className="text-3xl font-black text-slate-900">₹{(virtualBalance || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</h2>}
          </div>
          ...
          <div className="glass-card rounded-3xl p-8 bg-white border-slate-100 shadow-sm">
            ...
            {loading ? <Skeleton className="h-8 w-16" /> : <h2 className="text-3xl font-black text-slate-900">{totalSharesPurchased}</h2>}
          </div>
            ...
            <table className="w-full text-left">
              ...
              <tbody>
                {loading ? (
                  [1,2,3].map(i => (
                    <tr key={i}>
                      <td colSpan="6" className="px-8 py-6"><Skeleton className="h-12 w-full" /></td>
                    </tr>
                  ))
                ) : (
                  holdings.map((h) => (
                    <tr key={h.ticker} className="border-b border-border/30 hover:bg-white/5 transition-colors">
                      ...
                      <td className="px-8 py-6 font-bold text-slate-600">₹{(h.avgBuyPrice || 0).toFixed(2)}</td>
                      <td className="px-8 py-6 font-black text-blue-600">₹{((h.shares || 0) * (h.avgBuyPrice || 0)).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
                      <td className="px-8 py-6 font-black text-slate-900">₹{(h.currentPrice || 0).toFixed(2)}</td>
                      <td className={clsx("px-8 py-6 font-black text-right", (h.pnl || 0) >= 0 ? "text-emerald-500" : "text-red-500")}>
